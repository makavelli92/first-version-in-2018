...
namespace LevelStrategy.BL
{
    public static class Worker
    {
        ...
        public static void StartStrategy(Bars bars, Mutex mtx)
        {
            ...
            Strategy.FindSignal(bars, EventSignal, mtx);
        ...
        public static void EventSignal(object e, SignalData data)
        {
            ...
            {// перезаходим в метод потоком формы, чтобы не было исключения
                MainForm.grid.Invoke(new Action(() => {
                    ...
                    //{
                    //    SmtpClientHelper.SendEmail($@"Сигнал получен для {data.NameSecurity}. Тип - {data.SignalType}. Уровень - {data.Level}. Bремя - {DateTime.Now}");
                    //}
        ...
        private static void ChangeColorCells(DataGridViewRowCollection rowsCollection)
        {
            ...
            {
                if (i.Cells[8].Value != null && (i.DefaultCellStyle.BackColor == Color.Cyan || i.DefaultCellStyle.BackColor == Color.Brown) && (DateTime.Parse(DateTime.Now.ToShortTimeString()) - DateTime.Parse(i.Cells[8].Value.ToString())).TotalMinutes > 5)
                {
                    if (i.DefaultCellStyle.BackColor == Color.Brown)
