...
function main()
	...
	local Command = "";
	while not IsStop do
		--PrintDbgStr('i work'..os.date("%H:%M:%S"))
		...
		if Command ~= "" then
			...
			PrintDbgStr('Accept command'..tostring(Command))
			local temp = {}
			local i = 1
			local str = ""
			...
			PrintDbgStr('after split'..tostring(#temp))
			...
			elseif(temp[1] == 'GetPositions') then
				GetPosition();
			elseif(temp[1] == 'GetVolumes') then
				GetVolumes(temp);
			...
			elseif(temp[1] == 'GarbageCollector') then
				 Collect();
			end
			Command = "";
			PrintDbgStr('Clear Command')
		end
		--PrintDbgStr('Now sleep'..os.date("%H:%M:%S"))
	sleep(1000)
	end
end

function GetVolumes(temp)
table.remove(temp,1);
	
	i = #temp / 3
	local str = 'GetVol'..';'
	for j = 1, i,1 do
		str = str..GetVolume(temp[(j - 1) * 3 + 1], temp[(j - 1) * 3 + 2], temp[(j - 1) * 3 + 3])..';'
	end
	QluaCSharpConnector.SendCandle(str);
end

function GetVolume(class,security,interval,count)
	
	local ds;
    ds = CreateDataSource(class,security,tonumber(interval))
	local str = security..';'..ds:V(ds:Size() - 1)..';'..os.date('%Y-%m-%d %H:%M:%S', os.time(ds:T(ds:Size() - 1)))
	return str;
end

function Collect()
	PrintDbgStr('Un collect start count memmory')
	message('Un collect start count memmory'..tostring(collectgarbage ('count')))
   message(tostring(collectgarbage ('collect')))
   message('Aftetry Send Candler collect memmory used'..tostring(collectgarbage ('count')))
   PrintDbgStr('Aftetry Send Candler collect memmory')
end
...
function RequestCandle(temp)
	...
	PrintDbgStr('AfterRequestCandle')
end
...
function GetCandle(class,security,interval,count) 
	...
	PrintDbgStr('ain getCandle')
	local countCandle = -1;
	local ds;
    ds = CreateDataSource(class,security,tonumber(interval))
	ds:SetEmptyCallback()
	message('ds Calculate'..tostring(ds:Size()));
	PrintDbgStr('ds Calculate'..tostring(ds:Size()))
	if(ds:Size() == 0) then
		local tmpcount = 0
		local iteration = 0;
		repeat
...
function GetPosition()
	...
	local str = 'Positions';
		for i = 0,getNumberOf("depo_limits") - 1 do
			...
			if (getItem("depo_limits",i).limit_kind == 2) and (getItem("depo_limits",i).currentbal ~= 0)--(bit.test(getItem("stop_orders",i).flags, 0)) 
			then
					local count = 0;
					local dateStop = '-'
					local lastPrice = 0;
					local stopLimit = -1;
					local countLots = 0;
					lastPrice = getParamEx('TQBR', tostring(getItem("depo_limits",i).sec_code), "LAST").param_value
					if (lot == 0) then
						lastPrice = getParamEx('SPBFUT', tostring(getItem("depo_limits",i).sec_code), "LAST").param_value
					end
					for j = 0,getNumberOf("stop_orders") - 1 do
						if(getItem("stop_orders",j).sec_code == getItem("depo_limits",i).sec_code) and (bit.test(getItem("stop_orders",j).flags, 0)) then
						
									local lot = tonumber(getParamEx('TQBR', tostring(getItem("stop_orders",j).sec_code), "LOTSIZE").param_value)
									
										if (lot == 0) then
											lot = tonumber(getParamEx('SPBFUT', tostring(getItem("depo_limits",j).sec_code), "LOTSIZE").param_value)
											
										end;
									count = count + (tonumber(getItem("stop_orders",j).qty) * lot);
									dateStop = (getItem("stop_orders",j).ordertime)
									
									if((getItem("depo_limits",i).currentbal) > 0) then
										if(stopLimit == -1 and getItem("stop_orders",j).condition_price2 > 0) then
											stopLimit = (getItem("stop_orders",j).condition_price2)
											--countLots = count / lot
										elseif(stopLimit < (getItem("stop_orders",j).condition_price2)) then
											stopLimit = (getItem("stop_orders",j).condition_price2)
											--countLots = count / lot
										end
									elseif((getItem("depo_limits",i).currentbal) < 0) then
										if(stopLimit == -1 and getItem("stop_orders",j).condition_price2 > 0) then
											stopLimit = (getItem("stop_orders",j).condition_price2)
											--countLots = count / lot
										elseif(stopLimit > (getItem("stop_orders",j).condition_price2)) then
									...
									countLots = count / lot
									message('qty  '.. tostring(lastPrice))
						end;
					end;
					local pChange = 0;
					if(tonumber(getItem("depo_limits",i).openbal) ~= 0) then
						pChange = tonumber(getParamEx('TQBR', getItem("depo_limits",i).sec_code, "LASTCHANGE").param_value)
						if (pChange == 0) then
							pChange = tonumber(getParamEx('SPBFUT', getItem("depo_limits",i).sec_code, "LASTCHANGE").param_value)
						end
					end
					str = str..';'..((tostring(getItem("depo_limits",i).sec_code ..';'.. getItem("depo_limits",i).currentbal) ..';'.. getItem("depo_limits",i).awg_position_price
									..';'..count..';'..dateStop..';'..tostring(lastPrice)..';'..pChange..';'..getItem("depo_limits",i).openbal)..';'..stopLimit..';'..math.abs(countLots))
			end;
		end;
	QluaCSharpConnector.SendCandle(str)
	Collect()
